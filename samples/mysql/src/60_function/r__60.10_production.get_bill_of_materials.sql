CREATE PROCEDURE production.get_bill_of_materials (
...
BEGIN
    WITH RECURSIVE bom_cte AS (
        SELECT b.product_assembly_id
              ,b.component_id
              ,CAST(p.name AS CHAR(50))    AS component_desc
              ,b.per_assembly_qty
              ,p.standard_cost
              ,p.list_price
              ,b.bom_level
    ...
    SELECT c.product_assembly_id
          ,c.component_id
          ,c.component_desc
          ,CAST(SUM(c.per_assembly_qty) AS DECIMAL(8, 2)) AS total_quantity
          ,c.standard_cost
          ,c.list_price
          ,c.bom_level
          ,c.recursion_level
