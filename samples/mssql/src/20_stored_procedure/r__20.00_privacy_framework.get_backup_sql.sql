...
CREATE OR ALTER PROCEDURE [privacy_framework].[get_backup_sql](
        @schema_name sysname,
        @table_name sysname,
...
BEGIN
    ...
    IF NOT EXISTS(SELECT 1
        ...
        BEGIN
        SELECT @THROW_ERROR_MESSAGE = 'Error generating Backup SQL: Table ' + QUOTENAME(@schema_name) + '.' + QUOTENAME(@table_name) + ' from privacy_framework.table_configuration does not exist';
    ...
    IF NOT EXISTS(SELECT 1
        ...
        BEGIN
        SELECT @THROW_ERROR_MESSAGE = 'Error generating Backup SQL: Account ID column ' + QUOTENAME(@table_account_id_column_name) + ' for table ' + QUOTENAME(@schema_name) + '.' + QUOTENAME(@table_name) + ' from privacy_framework.table_configuration does not exist';
        THROW 50000, @THROW_ERROR_MESSAGE, 1;    
        RETURN @return_value;                             
        END;
    ...
    SELECT @sql_backup = 'IF OBJECT_ID(''' + @backup_table_name + ''') IS NULL BEGIN ' +
                         ...
                         'CREATE CLUSTERED INDEX ci_backup_temp on ' + @backup_table_name + ' (' + @table_account_id_column_name + ') ON FRAMEWORK_UNPART; ' +
                         'DROP INDEX ' + @backup_table_name + '.' + QUOTENAME('ci_backup_temp') + '; ' +
                         'END; ';

    --Insert the data to backup
    SELECT @sql_backup = @sql_backup + ' INSERT INTO ' + @backup_table_name + ' SELECT O.* FROM ' + QUOTENAME(@schema_name) + '.' + QUOTENAME(@table_name) + ' AS O ';
		SELECT @sql_backup = @sql_backup + ' WHERE O.' + QUOTENAME(@table_account_id_column_name) + ' = @account_id';
