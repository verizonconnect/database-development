...
CREATE OR ALTER PROCEDURE [privacy_framework].[obfuscate_account]
(	@account_id INT,
	@debug_ind BIT = 0
)
AS
BEGIN
    ...
    IF (@debug_ind = 0)
        ...
        BEGIN CATCH
            ...
            EXEC @return_value = privacy_framework.obfuscate_account_error @account_id = @account_id, 
    ...
    EXEC privacy_framework.set_obfuscation_account_status @account_id = @account_id, @status_id = @status_inprogress, @debug_ind = @debug_ind;

    -- Step 3
    -- Loop through the tables for subject level obfuscation
    SELECT @log_message = 'AccountID: ' + cast(@account_id as VARCHAR(10)) + ' | Getting tables to obfuscate';
    EXEC privacy_framework.add_obfuscation_log @message = @log_message, @debug_ind = @debug_ind;
	...
	WHILE (@@fetch_Status = 0)
		BEGIN
        EXEC privacy_framework.set_obfuscation_account_table_status @account_id = @account_id, 
        ...
        SELECT @log_message = 'AccountID: ' + cast(@account_id as VARCHAR(10))  + ' Table : ' + QUOTENAME(@loop_schema_name) + '.' + QUOTENAME(@loop_table_name) + 
                ' | Getting obfuscation SQL';
        EXEC privacy_framework.add_obfuscation_log @message = @log_message, @debug_ind = @debug_ind;

        BEGIN TRY
            EXEC @return_value = privacy_framework.get_obfuscation_sql @schema_name = @loop_schema_name, @table_name = @loop_table_name, @sql_obfuscation = @sql_obfuscation OUTPUT;
        END TRY
        BEGIN CATCH
            SELECT @Error_Number = ERROR_NUMBER();
            SELECT @Error_Message = ERROR_MESSAGE();
            ...
            EXEC @return_value = privacy_framework.obfuscate_main_error @account_id = @account_id, 
                                                        @schema_name = @loop_schema_name, @table_name = @loop_table_name, @sqlerrm = @THROW_ERROR_MESSAGE, @debug_ind = @debug_ind;
            RETURN @return_value;
        END CATCH;     
        ...
        BEGIN TRY
            EXEC @return_value = privacy_framework.get_backup_sql @schema_name = @loop_schema_name, @table_name = @loop_table_name, @sql_backup = @sql_backup OUTPUT;
        END TRY
        BEGIN CATCH
            SELECT @Error_Number = ERROR_NUMBER();
            SELECT @Error_Message = ERROR_MESSAGE();
        ...
        ELSE
            ...
            BEGIN TRY
                --Run the backup SQL
                EXECUTE SP_executesql @sql_backup, N'@account_id int', @account_id;
            END TRY
            BEGIN CATCH
                SELECT @Error_Number = ERROR_NUMBER();
                SELECT @Error_Message = ERROR_MESSAGE();
        ...
        ELSE
            ...
            BEGIN TRY
				...
				WHILE (@rows_updated > 0)
					BEGIN
                    EXECUTE SP_executesql @sql_obfuscation, N'@account_id int, @rows_updated int output', @account_id, @rows_updated OUTPUT;
        ...
        -- Step 3e
        -- Log the result
        EXEC privacy_framework.set_obfuscation_account_table_status @account_id = @account_id, @schema_name = @loop_schema_name, 
	...
	EXEC privacy_framework.set_obfuscation_account_status @account_id = @account_id,
							@status_id = @status_complete, @debug_ind = @debug_ind;
    SELECT @log_message = 'AccountID: ' + cast(@account_id as VARCHAR(10)) + 
                ' | Obfuscation Complete';
    EXEC privacy_framework.add_obfuscation_log @message = @log_message, @debug_ind = @debug_ind;
