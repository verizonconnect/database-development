...
CREATE OR ALTER PROCEDURE [privacy_framework].[get_obfuscation_sql](
        @schema_name sysname,
        @table_name sysname,
...
BEGIN
    ...
    WHERE tc.sch_nm = @schema_name
      ...
      AND EXISTS (SELECT 1 FROM privacy_framework.obfuscation_column_configuration cc
                    WHERE cc.sch_nm = tc.sch_nm AND cc.tbl_nm = tc.tbl_nm);
    ...
    IF NOT EXISTS(SELECT 1
        ...
        BEGIN
        SELECT @THROW_ERROR_MESSAGE = 'Error generating Obfuscation SQL: Table ' + QUOTENAME(@schema_name) + '.' + QUOTENAME(@table_name) + ' from privacy_framework.table_configuration does not exist';
    ...
    IF EXISTS(SELECT 1
        ...
        BEGIN
        SELECT @is_delete = 1;
        SELECT @sql_obfuscation = @sql_obfuscation + 'DELETE top(' + cast(@table_batch_size as varchar(10)) + ') O FROM ' + QUOTENAME(@schema_name) + '.' + QUOTENAME(@table_name) + ' AS O ';
        END;
    ELSE
        BEGIN
        SELECT @sql_obfuscation = @sql_obfuscation + 'UPDATE top(' + cast(@table_batch_size as varchar(10)) + ') O SET ';
        ...
        WHILE (@@fetch_Status = 0)
            ...
            IF @column_sql IS NULL OR @character_maximum_length IS NULL OR @character_maximum_length = -1
                BEGIN
                ...
                SELECT @sql_obfuscation = @sql_obfuscation + ' O.' + QUOTENAME(@column) + ' = ' + @column_sql + ',';
                END;
            ELSE
                BEGIN
                --Limit the updated value to the column length
                SELECT @sql_obfuscation = @sql_obfuscation + ' O.' + QUOTENAME(@column) + ' = left(' + @column_sql + ', ' + cast(@character_maximum_length as VARCHAR(10)) + '),';
                END;
            FETCH NEXT FROM Column_Loop INTO @column, @column_sql;
            END;
            ...
            SELECT @sql_obfuscation = @sql_obfuscation + ', O.' + QUOTENAME(@table_modified_utc_when_column_name) + ' = SYSUTCDATETIME() ';
            SELECT @sql_obfuscation = @sql_obfuscation + ', O.' + QUOTENAME(@table_obfuscated_ind_column_name) + ' = 1 ';
            SELECT @sql_obfuscation = @sql_obfuscation + ' FROM ' + QUOTENAME(@schema_name) + '.' + QUOTENAME(@table_name) + ' AS O ';
        END;

	SELECT @sql_obfuscation = @sql_obfuscation + ' WHERE O.' + QUOTENAME(@table_account_id_column_name) + ' = @account_id';

    SELECT @sql_obfuscation = @sql_obfuscation + ' AND COALESCE(O.' + QUOTENAME(@table_obfuscated_ind_column_name) + ', 0) = 0;';
